...
namespace OBWpnTypes
{
    ...
    inline OBWpnType createMachineGun()
    {
        ...
        {
            mWpn.shotProjectile(
            mGame.getFactory().createPJBullet(mShooter, mPos, mDeg));
    ...
    inline OBWpnType createPlasmaBolter()
    {
        ...
        {
            mWpn.shotProjectile(
            mGame.getFactory().createPJBoltPlasma(mShooter, mPos, mDeg));
    ...
    int mFanCount = 0, float mStep = 12.f)
    {
        ...
        [mFanCount, mStep](OBWpnType& mWpn, OBGame& mGame, OBCActorND* mShooter,
                const Vec2i& mPos, float mDeg, const Vec2f& mMuzzlePxPos)
        {
            fanShoot(&OBFactory::createPJBulletPlasma, mWpn, mGame, mShooter,
            mPos, mDeg, mFanCount, mStep, 40.f);
    ...
    inline OBWpnType createEPlasmaStarGun(int mFanCount = 0, float mStep = 12.f)
    {
        ...
        [mFanCount, mStep](OBWpnType& mWpn, OBGame& mGame, OBCActorND* mShooter,
                const Vec2i& mPos, float mDeg, const Vec2f& mMuzzlePxPos)
        {
            fanShoot(&OBFactory::createPJStarPlasma, mWpn, mGame, mShooter,
            mPos, mDeg, mFanCount, mStep, 40.f);
            mGame.createPMuzzlePlasma(16, mMuzzlePxPos);
        }};
    }
    inline OBWpnType createPlasmaCannon()
    {
        ...
        {
            mWpn.shotProjectile(
            mGame.getFactory().createPJCannonPlasma(mShooter, mPos, mDeg));
    ...
    inline OBWpnType createRocketLauncher()
    {
        ...
        {
            mWpn.shotProjectile(
            mGame.getFactory().createPJRocket(mShooter, mPos, mDeg));
    ...
    inline OBWpnType createGrenadeLauncher()
    {
        ...
        {
            mWpn.shotProjectile(
            mGame.getFactory().createPJGrenade(mShooter, mPos, mDeg));
            mGame.createPCaseRocket(1, mMuzzlePxPos, mDeg);
    ...
    inline OBWpnType createShockwaveGun()
    {
        ...
        {
            mWpn.shotProjectile(
            mGame.getFactory().createPJShockwave(mShooter, mPos, mDeg, 3));
