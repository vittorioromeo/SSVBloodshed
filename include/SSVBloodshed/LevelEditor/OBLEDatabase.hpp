...
namespace ob
{
template <typename TGame, typename TLevel, typename TTile, typename TFactory>
...
public:
    struct Entry
    {
        ...
        const decltype(defaultParams)& mDefaultParams, decltype(spawn) mSpawn)
...
public:
    inline OBLEDatabaseImpl(OBAssets& mAssets) : a(mAssets)
    {
        ...
        [this](TLevel&, TTile& mT, const Vec2i& mP)
        {
            f->createETurretRocket(mP, getDir8FromDeg(getP<float>(mT, "rot")));
        });
        ...
        {
            int mask{getWallMask(
            mL, OBLETType::LETWall, mT.getX(), mT.getY(), mT.getZ())};
        ...
        {
            int mask{getWallMask(
            mL, OBLETType::LETWallD, mT.getX(), mT.getY(), mT.getZ())};
            f->createFloor(mP, true);
        ...
        {
            int mask{getWallMask(
            mL, OBLETType::LETDoor, mT.getX(), mT.getY(), mT.getZ())};
            f->createFloor(mP, true);
        ...
        {
            int mask{getWallMask(
            mL, OBLETType::LETDoorG, mT.getX(), mT.getY(), mT.getZ())};
            f->createFloor(mP, true);
            f->createDoorG(mP, *a.doorGBitMask[mask], getP<bool>(mT, "open"));
        });
        ...
        {
            int mask{getWallMask(
            mL, OBLETType::LETDoorR, mT.getX(), mT.getY(), mT.getZ())};
            f->createFloor(mP, true);
            f->createDoorR(mP, *a.doorRBitMask[mask], getP<bool>(mT, "open"));
        });
        ...
        [this](TLevel&, TTile& mT, const Vec2i& mP)
        {
            ...
            getP<float>(mT, "delaySpawn"), getP<int>(mT, "spawnCount"));
        },
        ...
        [this](TLevel&, TTile& mT, const Vec2i& mP)
        {
            ...
            getPE<IdAction>(mT, "action"), getP<bool>(mT, "playerOnly"));
        },
        {{"action", "IdAction"}});
        ...
        {{"id", 0}, {"rot", 0}, {"blockFriendly", true}, {"blockEnemy", true},
        {"forceMult", 100.f}},
        [this](TLevel&, TTile& mT, const Vec2i& mP)
        {
            f->createForceField(mP, getP<int>(mT, "id"),
            getDir8FromDeg(getP<float>(mT, "rot")),
            getP<bool>(mT, "blockFriendly"), getP<bool>(mT, "blockEnemy"),
        ...
        {{"id", 0}, {"rot", 0}}, [this](TLevel&, TTile& mT, const Vec2i& mP)
        {
            f->createBooster(mP, getP<int>(mT, "id"),
    ...
    const sf::IntRect& mIntRect,
    const std::map<std::string, ssvj::Val>& mDefaultParams, const T& mSpawn,
