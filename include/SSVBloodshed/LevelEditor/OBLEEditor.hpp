...
public:
          ...
          guiCtx(assets, gameWindow, GUI::Style{*assets.obStroked})
    {
        ...
        formMenu =
        &guiCtx.create<GUI::Form>("MENU", Vec2f{400, 100}, Vec2f{64, 80});
        ...
        formInfo =
        &guiCtx.create<GUI::Form>("INFO", Vec2f{100, 100}, Vec2f{150, 80});
        ...
        for(int iY{brush.getTop()}; iY < brush.getBottom(); ++iY)
            for(int iX{brush.getLeft()}; iX < brush.getRight(); ++iX)
                if(sharedData.isTileValid(iX, iY, currentZ))
                    ...
                    &sharedData.getCurrentLevel().getTile(iX, iY, currentZ));
    }
        ...
        if(!sharedData.isCurrentLevelNull()) {
            for(auto& p : sharedData.getCurrentTiles()) {
                ...
                if(tile.getType() != OBLETType::LETNull)
                    tile.initGfxFromEntry(
                    sharedData.getDatabase().get(OBLETType(tile.getType())));
            }
    ...
    inline void draw()
    {
        ...
        {
            for(int i{-1}; i < 25; ++i) {
                auto& e(sharedData.getDatabase().get(OBLETType(ssvu::getMod(
                brush.getIdx() + i - 2, sharedData.getDatabase().getSize()))));
...
public:
    FormPack(GUI::Context& mCtx, OBLEEditor& mEditor)
        : GUI::Form{mCtx, "PACK", Vec2f{300.f, 300.f}, Vec2f{100.f, 100.f}},
          editor(mEditor), mainStrip(create<GUI::Strip>(
                           GUI::At::Top, GUI::At::Bottom, GUI::At::Bottom)),
          tboxName(
          mainStrip.create<GUI::TextBox>(getStyle().getBtnSize(56.f * 2))),
          shtrSectors(mainStrip.create<GUI::ChoiceShutter>(
          std::initializer_list<std::string>{},
          getStyle().getBtnSizePerChar(7))),
          tboxSectorIdx(
          mainStrip.create<GUI::TextBox>(getStyle().getBtnSizePerChar(7))),
    ...
    inline void refreshTile()
    {
        ...
        for(auto& p : tile->getParams()) {
            ...
            if(entry.isEnumParam(key)) {
                ...
                getStyle().getBtnSizePerChar(7)));
                choiceShutter.onChoiceSelected += [key, tile, &choiceShutter]
                {
                    tile->setParam(
                    key, ssvu::toStr(choiceShutter.getChoiceIdx()));
                };
                choiceShutter.getShutter().setScalingX(GUI::Scaling::Manual);
            ...
            else
            {
                ...
                if(p.second.is<bool>()) {
                    auto& checkBox(
                    strip.create<GUI::CheckBox>("on", p.second.as<bool>()));
...
public:
    FormParams(GUI::Context& mCtx, OBLEEditor& mEditor, int mX, int mY, int mZ)
