...
private:
    ...
    inline void refresh()
    {
        ...
        min.x = ssvu::getClamped(
        position.x - size / 2, bounds.left, bounds.left + bounds.width - size);
        min.y = ssvu::getClamped(
        position.y - size / 2, bounds.top, bounds.top + bounds.height - size);
