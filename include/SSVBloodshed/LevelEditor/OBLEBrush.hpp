...
namespace ob
{
	class OBLEBrush : public sf::Drawable
	{
		private:
			...
			inline void refresh()
			{
				center.x = ssvu::getClamped(position.x, bounds.left, (bounds.left + bounds.width - 1));
				center.y = ssvu::getClamped(position.y, bounds.top, (bounds.top + bounds.height - 1));

				min.x = ssvu::getClamped(position.x - size / 2, bounds.left, bounds.left + bounds.width - size);
				min.y = ssvu::getClamped(position.y - size / 2, bounds.top, bounds.top + bounds.height - size);

				max.x = ssvu::getClamped(position.x + std::ceil(size / 2.f), bounds.left + size, bounds.left + bounds.width);
				max.y = ssvu::getClamped(position.y + std::ceil(size / 2.f), bounds.top + size, bounds.top + bounds.height);
