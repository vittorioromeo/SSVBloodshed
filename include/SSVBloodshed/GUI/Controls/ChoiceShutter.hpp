    ...
    namespace GUI
    {
        ...
        private:
            ...
            inline void refreshChoices()
            {
                ...
                lblCount.setString("(" + ssvu::toStr(lb) + ":" +
                                   ssvu::toStr(ub) + ")/" +
        ...
        public:
            ...
            ChoiceShutter(Context& mContext,
                ...
                : Shutter{mContext, "", mSize}, choices{mChoices},
                  wsChoices(getShutter().create<Strip>(
                      At::Top, At::Bottom, At::Bottom)),
                  wsScroll(getShutter().create<Strip>(
                      At::Right, At::Left, At::Left)),
                  btnUp(wsScroll.create<Button>(
                      "^", getStyle().getBtnSquareSize())),
                  btnDown(wsScroll.create<Button>(
                      "v", getStyle().getBtnSquareSize())),
                  lblCount(wsScroll.create<Label>(""))
            {
                ...
                for(auto i(0u); i < choiceBtnsMax; ++i)
                {
                    auto& btn(wsChoices.create<Button>(
            ...
            inline int getChoiceIdx() const noexcept
            {
