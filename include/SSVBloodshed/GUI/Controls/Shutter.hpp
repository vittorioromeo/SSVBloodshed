    ...
    namespace GUI
    {
        ...
        private:
            ...
            inline void update(FT) override
            {
                ...
                if(!wsShutter.isAnyChildRecursive([](const Widget& mW)
                       {
                           return mW.isFocused();
                       }))
                    wsShutter.setExcludedRecursive(true);
                wsShutter.setFillColor(wsShutter.isFocused()
                                           ? getStyle().colorBaseFocused
                                           : getStyle().colorBaseUnfocused);
            }

        public:
            Shutter(Context& mContext, std::string mLabel, const Vec2f& mSize)
                : Widget{mContext},
                  wsBar(create<Strip>(At::Right, At::Left, At::Left)),
                  wsShutter(create<Strip>(At::Top, At::Bottom, At::Bottom)),
                  btnOpen(
                      wsBar.create<Button>("v", getStyle().getBtnSquareSize())),
                  lblLabel(wsBar.create<Label>(ssvu::mv(mLabel)))
            {
                ...
                wsShutter.attach(
                    At::Top, wsBar, At::Bottom, Vec2f{0.f, getStyle().padding});
            ...
            inline Shutter& operator+=(Widget& mWidget)
            {
                wsShutter += mWidget;
                return *this;
            }
            inline Shutter& operator+=(
