...
namespace ob
{
    namespace GUI
    {
        ...
        inline void Widget::updateInput()
        {
                    ...
                    }))
            {
                ...
                recurseParents([this, &fail](Widget& mW)
                    {
                        if(!fail &&
                            !ssvs::isInAABB(mW.viewBoundsMin, mW.viewBoundsMax,
                                getMousePos(), 2.f))
                            fail = true;
                    });
                if(fail) return;
            ...
            // If the context is already busy with another widget, do not
            // process
            // input
            if(context.isBusy() && context.busyWith != this) return;

            auto processButton([this](bool mContextPressed, bool mPressed,
                ssvu::Delegate<void()>& onClick,
                ssvu::Delegate<void()>& onClickDown,
        ...
        {
            context.render(
                parent != nullptr ? &parent->view : nullptr, mDrawable);
        }
