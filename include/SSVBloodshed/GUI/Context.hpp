...
namespace GUI
{
    ...
    private:
        ...
        inline void render(sf::View* mView, const sf::Drawable& mDrawable)
        {
            ...
            mView != nullptr ? *mView : gameWindow.getRenderWindow().getView());
        ...
        {
            SSVU_ASSERT_STATIC(
            ssvu::isBaseOf<Widget, T>(), "T must be derived from Widget");
        ...
        {
            // If the context is busy (dragging, resizing, editing...), do not
            // change focus
            if(isBusy()) return;
            ...
            if(found != nullptr) {
                ...
                {
                    if(mW.isHovered() || mW.isPressedAny()) foundAny = true;
                });
            }
            ...
            {
                if(mW.isPressedAny() && mW.depth > found->depth) found = &mW;
            });
    ...
    public:
        inline Context(
        OBAssets& mAssets, ssvs::GameWindow& mGameWindow, const Style& mStyle)
        ...
        inline void update(FT mFT)
        {
            ...
            // Recursively remove all dead widgets from children and set "not
            // recalculated"
            for(auto& w : children)
            ...
            // If mouse buttons are not down or the `busyWith` widget is dead,
            // stop being busy
            if((!mouseLDown && !mouseRDown) ||
               (busyWith != nullptr && decltype(widgets)::isDead(busyWith)))
                busyWith = nullptr;

            // Free dead widgets memory / initialize new widgets
            widgets.refresh();

            // Focus the correct widgets. If any is focused, set `focused` to
            // true.
            updateFocus();

            // Set `hovered` to false: if any widget is hovered, it will become
            // true
            hovered = false;
