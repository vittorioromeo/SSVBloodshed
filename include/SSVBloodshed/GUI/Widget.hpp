...
namespace GUI
{
    ...
    private:
        ...
        // Settings
        bool hidden{false};   // Controlled by hide/show: if true, it makes the
                              // widget implicitly invisible and inactive
        bool excluded{false}; // Controlled by setExcludedRecursive: if true, it
                              // makes the widget implicitly invisible and
                              // inactive
        bool collapsed{false}; // Controlled by window collapsing: if true, it
                               // makes the widget implicitly invisible and
                               // inactive
        ...
        bool Vec2<bool>::*mDimensionBool)
        {
            ...
            if(this->*mScaling == Scaling::FitToChildren) {
                ...
                for(auto& w : children) {
                    ...
                    std::min(childBoundsMin.*mDimension, (w->*mGetterMin)());
                    childBoundsMax.*mDimension =
                    std::max(childBoundsMax.*mDimension, (w->*mGetterMax)());
                }

                (this->*mSetter)(childBoundsMax.*mDimension -
                                 childBoundsMin.*mDimension + padding * 2.f);
            }
            else if(this->*mScaling == Scaling::FitToNeighbor)
            {
                if(neighbor != nullptr) {
                    ...
                    mGetterMax, mDimension, mDimensionBool);
                    (this->*mSetter)(
                    (((neighbor->*mGetterMax)() - (neighbor->*mGetterMin)()) *
                     (scalePercent / 100.f)) -
                    padding * 2.f);
                }
            }
            else if(this->*mScaling == Scaling::FitToParent)
            {
                if(parent != nullptr) {
                    ...
                    mGetterMax, mDimension, mDimensionBool);
                    (this->*mSetter)(
                    (((parent->*mGetterMax)() - (parent->*mGetterMin)()) *
                     (scalePercent / 100.f)) -
                    padding * 2.f);
                }
            }
        ...
        inline void updateRecursive(FT mFT)
        {
            // Recalculate depth
            depth =
            parent == nullptr ? 0 : parent->depth + ssvu::toInt(container);

            update(mFT);

            // Recalculate size and position
            recalcSizeSmart(&Widget::scalingX, &Widget::setWidth,
            &Widget::getLeft, &Widget::getRight, &Vec2f::x, &Vec2<bool>::x);
            recalcSizeSmart(&Widget::scalingY, &Widget::setHeight,
            &Widget::getTop, &Widget::getBottom, &Vec2f::y, &Vec2<bool>::y);
    ...
    public:
        ...
        inline Widget(
        Context& mContext, const Vec2f& mPosition, const Vec2f& mHalfSize)
        ...
        template <bool TIncludeCaller = true, bool TReverse = false, typename T>
        inline void recurseChildrenBF(const T& mFunc)
        {
            ...
            ssvu::sortStable(hierarchy, [](const Widget* mA, const Widget* mB)
            {
        ...
        inline bool isAnyParentRecursive(const T& mFunc) const
        {
            if(TIncludeCaller)
                if(mFunc(*this)) return true;
            if(parent != nullptr)
                if(parent->isAnyParentRecursive<true, T>(mFunc)) return true;
            return false;
        }
        ...
        // An hidden widget is both invisible and inactive (should be controlled
        // by user)
        ...
        // An excluded widget is both invisible and inactive (should be used to
        ...
        {
            return visible && !isHidden() && !isExcluded() && !isCollapsed();
        }
