...
namespace ob
{
    ...
    public:
        ...
        {
            SSVU_ASSERT(
                database != nullptr && mPath.exists<ssvufs::Type::File>());
            setPath(mPath);

            try
            {
                pack = ssvj::fromFile(currentPath).as<OBLEPack>();
            }
            catch(...)
            {
                ssvu::lo("Fatal error") << "Failed to load pack" << std::endl;
            }
        }
        inline void savePack(const ssvufs::Path& mPath)
        {
            ...
            try
            {
                ssvj::Val{pack}.writeToFile<ssvj::WSMinified>(currentPath);
            }
            catch(...)
            {
                ssvu::lo("Fatal error") << "Failed to save pack" << std::endl;
            }
        }

        inline void setDatabase(
            OBLEDatabase& mDatabase, OBGame* mGame = nullptr) noexcept
        {
            database = &mDatabase;
        ...
        {
            SSVU_ASSERT(database != nullptr);
            if(!isSectorValid(mIdx))
                ssvu::lo("Warning") << "Sector of idx " << mIdx
                                    << " is not valid\n";
        ...
        {
            if(!isLevelValid(mX, mY))
                ssvu::lo("Warning") << "Level (" << mX << ";" << mY
                                    << ") is not valid\n";
        ...
        inline OBLESector& getCurrentSector() const noexcept
        {
            SSVU_ASSERT(currentSector != nullptr);
            return *currentSector;
        }
        inline OBLELevel& getCurrentLevel() const noexcept
        {
            SSVU_ASSERT(currentLevel != nullptr);
            return *currentLevel;
        }
        inline decltype(currentLevel->getTiles()) getCurrentTiles() const
        {
            return getCurrentLevel().getTiles();
        }
        inline const ssvufs::Path& getCurrentPath() const noexcept
        {
            return currentPath;
        }

        inline bool isCurrentLevelNull() const noexcept
        {
        ...
        inline bool isTileValid(int mX, int mY, int mZ) const noexcept
        {
            return getCurrentLevel().isValid(mX, mY, mZ);
        }

        inline int getCurrentSectorIdx() const noexcept
        {
            SSVU_ASSERT(currentSector != nullptr);
            return currentSectorIdx;
        }
        inline int getCurrentLevelX() const noexcept
        {
            SSVU_ASSERT(currentLevel != nullptr);
            return currentLevelX;
        }
        inline int getCurrentLevelY() const noexcept
        {
            SSVU_ASSERT(currentLevel != nullptr);
