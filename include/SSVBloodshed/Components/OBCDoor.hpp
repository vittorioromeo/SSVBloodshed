...
namespace ob
{
    ...
    protected:
        ...
        inline void setOpen(bool mOpen) noexcept
        {
            ...
            body.setGroups(
                !openStatus, OBGroup::GSolidGround, OBGroup::GSolidAir);
        }

    public:
        OBCDoorBase(Entity& mE, OBCPhys& mCPhys, OBCDraw& mCDraw,
            bool mOpen = false) noexcept : OBCActor{mE, mCPhys, mCDraw},
                                           openStatus{mOpen}
        {
            setOpen(openStatus);
        }
    ...
    public:
        OBCDoor(Entity& mE, OBCPhys& mCPhys, OBCDraw& mCDraw,
            OBCIdReceiver& mCIdReceiver, bool mOpen = false) noexcept
            : OBCDoorBase{mE, mCPhys, mCDraw, mOpen},
              cIdReceiver(mCIdReceiver)
        {
            cIdReceiver.onActivate += [this](IdAction mIdAction)
            {
                if(mIdAction == IdAction::Toggle)
                    toggle();
                else if(mIdAction == IdAction::Open)
                    open();
                else if(mIdAction == IdAction::Close)
                    close();
            };
        }
    };
    ...
    public:
        OBCDoorG(Entity& mE, OBCPhys& mCPhys, OBCDraw& mCDraw,
            bool mOpen = false) noexcept
            : OBCDoorBase{mE, mCPhys, mCDraw, mOpen}
        {
        }
        inline void update(FT) override
        {
            if(!triggered && game.isLevelClear())
            {
                toggle();
                triggered = true;
            }
    ...
    public:
        OBCDoorR(Entity& mE, OBCPhys& mCPhys, OBCDraw& mCDraw,
            bool mOpen = false) noexcept
            : OBCDoorBase{mE, mCPhys, mCDraw, mOpen}
        {
        }
        inline void update(FT) override
        {
            if(!triggered && manager.getEntityCount(OBGroup::GTrapdoor) <= 0)
            {
                toggle();
                triggered = true;
            }
