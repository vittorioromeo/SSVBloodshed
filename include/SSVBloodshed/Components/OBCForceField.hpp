...
public:
    OBCForceField(Entity& mE, OBCPhys& mCPhys, OBCDraw& mCDraw,
    OBCIdReceiver& mCIdReceiver, Dir8 mDir, bool mBlockFriendly,
    bool mBlockEnemy, float mForceMult) noexcept
    : OBCActor{mE, mCPhys, mCDraw},
      cIdReceiver(mCIdReceiver),
      blockFriendly{mBlockFriendly},
      blockEnemy{mBlockEnemy},
      ...
      animation{assets.aForceField}
    {
        ...
        segment = {
        ssvs::getOrbitRad(body.getPosition(), rad + ssvu::piHalf, 1500.f),
        ssvs::getOrbitRad(body.getPosition(), rad - ssvu::piHalf, 1500.f)};
        ...
        body.onDetection += [this](const DetectionInfo& mDI)
        {
            ...
            // If this force field blocks friendlies or enemies and the detected
            // body matches...
            if((blockFriendly && mDI.body.hasGroup(OBGroup::GFriendly)) ||
               (blockEnemy && mDI.body.hasGroup(OBGroup::GEnemy)))
            {
                bool isMoving{
                mDI.body.getVelocity().x != 0 || mDI.body.getVelocity().y != 0};

                // Check if the body is "inside" the force field (check if it's
                ...
                if(!segment.isPointLeft(Vec2f(mDI.body.getPosition()))) {
                    ...
                    // If it's moving and it's not inside, treat the collision
                    // as a solid one
                    if(isMoving &&
                       isRadBlocked(ssvs::getRad(mDI.body.getVelocity())))
                        mDI.body.resolvePosition(
                        ssvsc::Utils::getMin1DIntersection(
                        mDI.body.getShape(), body.getShape()));
                }
                else if(!isMoving)
                    mDI.body.applyAccel(dirVec * -5.f * forceMult);
            }
    ...
    inline bool isRadBlocked(float mRad) const noexcept
    {
        return ssvu::getDistRad(mRad, rad) <= ssvu::piHalf;
    }
};
}
