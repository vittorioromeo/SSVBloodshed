...
namespace ob
{
	class OBCBoid : public OBCActorNoDrawBase
	{
		private:
			...
			{
				mTarget = cPhys.getPos<float>() - mTarget;
			...
			{
				Vec2f distance{mTargetPos - cPhys.getPos<float>()};
		...
		public:
			...
			{
				body.applyForce(getPursuitForce(mTarget.getPos<float>(), mTarget.getVel(), mPredictionMult, mSlowRadius) * mForceMult);
			}
			inline void evade(const OBCPhys& mTarget, float mForceMult = 0.02f) noexcept
			{
				body.applyForce(getEvadeForce(mTarget.getPos<float>(), mTarget.getVel()) * mForceMult);
			}
