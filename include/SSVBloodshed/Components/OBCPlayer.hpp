...
namespace ob
{
        ...
        public:
            inline void update(float mFT) noexcept
            {
                if(time <= 0)
                    ...
                    time -= mFT * ssvu::getClampedMax(1 + count * 0.05f, 1.5f);
            }
    ...
    private:
        ...
        std::vector<WeaponData> weapons{
            ...
            {OBWpnTypes::createPlasmaCannon(), assets.gunPCannon,
                "plasma cannon"},
            {OBWpnTypes::createRocketLauncher(), assets.p2Gun,
                "rocket launcher"},
            {OBWpnTypes::createGrenadeLauncher(), assets.p2Gun,
                "grenade launcher"},
            {OBWpnTypes::createShockwaveGun(), assets.wpnShockwave,
                "shockwave gun"}};
    ...
    public:
        OBCPlayer(Entity& mE, OBCPhys& mCPhys, OBCDraw& mCDraw,
            OBCKillable& mCKillable, OBCWielder& mCWielder,
            OBCWpnController& mCWpnController) noexcept
            : OBCActor{mE, mCPhys, mCDraw},
              cKillable(mCKillable),
              cWielder(mCWielder),
              cDir8(mCWielder.getCDir8()),
              cWpnController(mCWpnController)
        {
            ...
            getEntity().addGroups(OBGroup::GFriendly,
                OBGroup::GFriendlyKillable, OBGroup::GPlayer);
            body.addGroups(OBGroup::GSolidGround, OBGroup::GSolidAir,
                OBGroup::GFriendly, OBGroup::GKillable,
                OBGroup::GFriendlyKillable, OBGroup::GOrganic,
                OBGroup::GPlayer);
            body.addGroupsToCheck(OBGroup::GSolidGround);
            body.onResolution += [this](const ResolutionInfo& mRI)
            {
                if(mRI.body.hasGroup(OBGroup::GLevelBound)) checkTransitions();
            };
        }
        ...
        {
            auto gridQuery(
                getGame().getWorld().getQuery<ssvsc::QueryType::RayCast>(
                    getCPhys().getPosI(), cDir8.getVec()));
            ...
            auto distDiff(
                maxDist - ssvs::getMag(rayDirVec) < 0.05f ? 0.f : 1.f);

            validShootingPos = getCPhys().getPosI() + ssvs::Vec2i(rayDirVec) -
                               ssvs::Vec2i(rayDirVec * distDiff);
        }
        ...
        inline void updateUsable()
        {
            ...
            for(auto& e : manager.getEntities(OBGroup::GUsable))
            {
                ...
                if(ssvs::getDistSquaredEuclidean(usableCPhys.getPosI(),
                       getCPhys().getPosI()) > 1300 * 1300)
                    continue;
        ...
        inline void attractShards()
        {
            ...
            {
                auto query(
                    cPhys.getWorld().getQuery<ssvsc::QueryType::Distance>(
                        cPhys.getPosI(), 3500));

                Body* b;
                while((b = query.next()) != nullptr)
                    if(b->hasGroup(OBGroup::GShard))
                        b->applyAccel(
                            Vec2f(cPhys.getPosI() - b->getPosition()) * 0.004f);
            }
            else
            {
                for(auto& e : manager.getEntities(OBGroup::GShard))
                {
                    ...
                    if(ssvs::getDistEuclidean(c.getPosI(), cPhys.getPosI()) >
                        6500)
                    {
                        if(ssvs::getMag(c.getVel()) < 650.f)
                            c.getBody().applyAccel(
                                Vec2f(cPhys.getPosI() - c.getPosI()) * 0.002f);
                    }
                    else
                        c.setVel(ssvs::getMClampedMax(
                            Vec2f(cPhys.getPosI() - c.getPosI()) / 1.5f,
                            400.f));
                }
            }
        }
        ...
        {
            if(cPhys.getLeft() + cPhys.getVel().x < 0)
                game.changeLevel(*this, -1, 0);
            else if(cPhys.getRight() + cPhys.getVel().x > levelWidthCoords)
                game.changeLevel(*this, 1, 0);
            else if(cPhys.getTop() + cPhys.getVel().y < 0)
                game.changeLevel(*this, 0, -1);
            else if(cPhys.getBottom() + cPhys.getVel().y > levelHeightCoords)
                game.changeLevel(*this, 0, 1);
        }

        inline void update(FT mFT) override
        {
            updateValidShootingPos();
            updateUsable();
            updateInput();
            attractShards();
            ...
            if(cWielder.isShooting())
                cWpnController.shoot(this, validShootingPos, cDir8.getDeg(),
        ...
        inline void bomb()
        {
            for(int k{0}; k < 5; ++k)
                for(int i{0}; i < 360; i += 360 / 16)
                    factory.createPJTestBomb(this, body.getPosition(),
                        cDir8.getDeg() + (i * (360 / 16)),
                        2.f - k * 0.2f + i * 0.004f,
                        4.f + k * 0.3f - i * 0.004f);
        }
        ...
        inline OBCHealth& getCHealth() noexcept
        {
            return cKillable.getCHealth();
        }
...
namespace ob
{
    inline void OBGameHUD::refresh(OBCPlayer& mP)
    {
        ...
        txtVM.setString(mP.currentUsable == nullptr
                            ? mP.weapons[mP.currentWpn].name
                            : mP.currentUsable->getMsg());

        txtCombo.setString(csC > 3 ? "x" + ssvu::toStr(csC) : "");
        txtCombo.setOrigin(ssvs::getGlobalHalfSize(txtCombo));
        ssvs::Vec2f offset(
            ssvu::getRndR<float>(-csC, csC), ssvu::getRndR<float>(-csC, csC));
        ssvs::cClamp(offset, -250.f, 250.f);
        txtCombo.setPosition(overlayCamera.getCenter() + offset / 10.f);

        auto c(txtCombo.getColor());
        auto k = ssvu::getMapEased<ssvu::Easing::Sine, ssvu::Easing::InOut>(
            cs.getTime(), 0.f, cs.getTimeMax(), 0.f, 230.f);
