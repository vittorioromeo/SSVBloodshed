...
public:
      ...
      cWpnController(mCWpnController)
    {
        ...
        getEntity().addGroups(
        OBGroup::GFriendly, OBGroup::GFriendlyKillable, OBGroup::GPlayer);
        body.addGroups(OBGroup::GSolidGround, OBGroup::GSolidAir,
        OBGroup::GFriendly, OBGroup::GKillable, OBGroup::GFriendlyKillable,
        OBGroup::GOrganic, OBGroup::GPlayer);
    ...
    {
        auto gridQuery(getGame().getWorld().getQuery<ssvsc::QueryType::RayCast>(
    ...
    inline void updateInput()
    {
        ...
        auto accel(
        ssvs::getResized(Vec2f(ix, iy), walkSpeed) - body.getVelocity());
    ...
    inline void updateUsable()
    {
        ...
        for(auto& e : manager.getEntities(OBGroup::GUsable)) {
            ...
            if(ssvs::getDistSquaredEuclidean(
               usableCPhys.getPosI(), getCPhys().getPosI()) > 1300 * 1300)
                continue;
    ...
    inline void attractShards()
    {
        if(!game.isLevelClear()) {
            auto query(cPhys.getWorld().getQuery<ssvsc::QueryType::Distance>(
            cPhys.getPosI(), 3500));
        ...
        else
        {
            for(auto& e : manager.getEntities(OBGroup::GShard)) {
                ...
                if(ssvs::getDistEuclidean(c.getPosI(), cPhys.getPosI()) > 6500)
                {
                    ...
                    Vec2f(cPhys.getPosI() - c.getPosI()) / 1.5f, 400.f));
            }
        }
    }
    ...
    inline void bomb()
    {
        for(int k{0}; k < 5; ++k)
            for(int i{0}; i < 360; i += 360 / 16)
                ...
                cDir8.getDeg() + (i * (360 / 16)), 2.f - k * 0.2f + i * 0.004f,
                4.f + k * 0.3f - i * 0.004f);
    }
...
inline void OBGameHUD::refresh(OBCPlayer& mP)
{
    ...
    txtVM.setString(mP.currentUsable == nullptr ? mP.weapons[mP.currentWpn].name
