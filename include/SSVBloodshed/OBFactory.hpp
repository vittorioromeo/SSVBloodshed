...
private:
    ...
    const Vec2i& mSize, float mSpeed, float mDeg, const sf::IntRect& mIntRect);
...
public:
    ...
    float mDelayStart = 0.f, float mDelaySpawn = 200.f, int mSpawnCount = 1);
    ...
    OBCActorND* mShooter, const Vec2i& mPos, float mDeg);
    Entity& createPJExplosion(
    OBCActorND* mShooter, const Vec2i& mPos, float mDeg, float mSpeed = 300.f);
    ...
    {
        auto& cp(createPJExplosion(mShooter, gt<OBCPhys>(mTpl).getPosI(), 0, 0)
        ...
        for(int i{0}; i < 360; i += 360 / mCount) {
            auto& cpMv(
            createPJExplosion(mShooter,
            gt<OBCPhys>(mTpl).getPosI() + Vec2i(ssvs::getVecFromDeg(i, 251.f)),
            i)
            .template getComponent<OBCProjectile>());
