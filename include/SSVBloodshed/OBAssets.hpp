...
namespace ob
{
    ...
    public:
        ...
        sf::IntRect pjStarPlasma, pjCannonPlasma, pjRocket,
            pjGrenade;                                      // Projectiles
        ...
        sf::IntRect fpj0, fpj1, fpj2, fpj3, fpj4, fpj5, fpj6, fpj7, fpj8,
            fpj9; // Booster frames
        ...
        // Particle data
        OBParticleData pdBloodRed, pdGibRed, pdExplosion, pdDebris,
            pdDebrisFloor;
        OBParticleData pdMuzzleBullet, pdMuzzlePlasma, pdMuzzleRocket, pdPlasma,
            pdElectric;
        OBParticleData pdSmoke, pdShard, pdCharge, pdHeal, pdForceField;
        OBParticleData pdCaseBullet, pdCaseRocket, pdShockwave,
            pdMuzzleShockwave;

#define WALLTSDECL(x)                                                      \
    sf::IntRect x##Single, x##Cross, x##V, x##H, x##CornerSW, x##CornerSE, \
        x##CornerNW, x##CornerNE, x##VEndS, x##VEndN, x##HEndW, x##HEndE,  \
        x##TS, x##TN, x##TW, x##TE;                                        \
...
#define WALLTS(x)                      \
    ...
    {                                  \
        ...
        T_TSSMALL(x##V);               \
        T_TSSMALL(x##H);               \
        T_TSSMALL(x##CornerSW);        \
        T_TSSMALL(x##CornerSE);        \
        T_TSSMALL(x##CornerNW);        \
        T_TSSMALL(x##CornerNE);        \
        T_TSSMALL(x##VEndS);           \
        T_TSSMALL(x##VEndN);           \
        T_TSSMALL(x##HEndW);           \
        T_TSSMALL(x##HEndE);           \
        ...
        T_TSSMALL(x##TW);              \
        T_TSSMALL(x##TE);              \
        x##BitMask[0] = &x##Single;    \
        x##BitMask[1] = &x##VEndS;     \
        x##BitMask[2] = &x##HEndW;     \
        x##BitMask[3] = &x##CornerSW;  \
        x##BitMask[4] = &x##VEndN;     \
        x##BitMask[5] = &x##V;         \
        x##BitMask[6] = &x##CornerNW;  \
        x##BitMask[7] = &x##TW;        \
        x##BitMask[8] = &x##HEndE;     \
        x##BitMask[9] = &x##CornerSE;  \
        x##BitMask[10] = &x##H;        \
        x##BitMask[11] = &x##TS;       \
        x##BitMask[12] = &x##CornerNE; \
        x##BitMask[13] = &x##TE;       \
        x##BitMask[14] = &x##TN;       \
        x##BitMask[15] = &x##Cross;    \
    } while(false);
            ...
            obBigStroked =
                &assetManager.get<ssvs::BitmapFont>("fontObBigStroked");
            ...
            T_TSSMALL(floorAlt1);
            T_TSSMALL(floorAlt2);
            T_TSSMALL(floorGrate);
            T_TSSMALL(floorGrateAlt1);
            T_TSSMALL(floorGrateAlt2);
            T_TSSMALL(pit);
            T_TSSMALL(trapdoor);
            T_TSSMALL(trapdoorPOnly);
            T_TSSMALL(pjBullet);
            T_TSSMALL(pjPlasma);
            T_TSSMALL(pjStar);
            T_TSSMALL(pjBulletPlasma);
            T_TSSMALL(pjStarPlasma);
            T_TSSMALL(pjCannonPlasma);
            T_TSSMALL(pjRocket);
            T_TSSMALL(pjGrenade);
            T_TSSMALL(eBall);
            T_TSSMALL(eBallFlying);
            T_TSSMALL(eTurret0);
            T_TSSMALL(eTurret1);
            T_TSSMALL(eTurret2);
            T_TSSMALL(eTurret3);
            T_TSSMALL(pPlateSingle);
            T_TSSMALL(pPlateMulti);
            T_TSSMALL(pPlateOnOff);
            T_TSSMALL(shard);
            ...
            T_TSSMALL(spawner);
            T_TSSMALL(forceArrow);
            T_TSSMALL(forceArrowMark);
            T_TSSMALL(bulletBooster);
            T_TSSMALL(bulletChanger);
            T_TSSMALL(pjShockwave);
            T_TSSMALL(wpnShockwave);
            ...
            // Animations
            ssvj::Val jAForceField{
                ssvj::fromFile("Data/Animations/forceField.json")};
            aForceField =
                ssvs::getAnimationFromJson(*tsSmall, jAForceField["on"]);

            ssvj::Val jABulletBooster{
                ssvj::fromFile("Data/Animations/bulletBooster.json")};
            aBulletBooster =
                ssvs::getAnimationFromJson(*tsSmall, jABulletBooster["on"]);

            // Particle data
            auto gpd([this](const std::string& mName)
                {
                    return ssvj::fromFile("Data/Particles/" + mName + ".json")
                        .as<OBParticleData>();
                });
...
#undef WALLTS
        ...
        inline const sf::IntRect& getFloorVariant() const noexcept
        {
            return ssvu::getRndI(0, 10) < 9
                       ? floor
                       : (ssvu::getRndI(0, 2) < 1 ? floorAlt1 : floorAlt2);
        }
        inline const sf::IntRect& getFloorGrateVariant() const noexcept
        {
            return ssvu::getRndI(0, 10) < 9
                       ? floorGrate
                       : (ssvu::getRndI(0, 2) < 1 ? floorGrateAlt1
                                                  : floorGrateAlt2);
        }
    };
}
