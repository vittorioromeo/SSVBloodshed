...
namespace ob
{
        ...
        const Vec2i& mPos, const Vec2i& mSize, int mDrawPriority, bool mStatic)
    {
        ...
        auto& cPhys(
            result.createComponent<OBCPhys>(game, mStatic, mPos, mSize));
        ...
        const Vec2i& mPos, const Vec2i& mSize, int mDrawPriority, float mHealth)
    {
        ...
        return ssvu::fwdAsTpl(gt<Entity>(tpl), gt<OBCPhys>(tpl),
            gt<OBCDraw>(tpl), cHealth, cKillable);
    }
        ...
        const Vec2i& mPos, const Vec2i& mSize, int mHealth)
    {
        ...
        return ssvu::fwdAsTpl(gt<Entity>(tpl), gt<OBCPhys>(tpl),
            gt<OBCDraw>(tpl), gt<OBCHealth>(tpl), gt<OBCKillable>(tpl), cEnemy);
    }
    auto OBFactory::createProjectileBase(OBCActorND* mShooter,
        const Vec2i& mPos, const Vec2i& mSize, float mSpeed, float mDeg,
        const IntRect& mIntRect)
    {
        ...
        float mPJDelay, int mShootCount)
    {
        ...
        gt<Entity>(tpl).createComponent<OBCTurret>(gt<OBCPhys>(tpl),
            gt<OBCDraw>(tpl), gt<OBCKillable>(tpl), mDir, mWpn, mShootDelay,
            mPJDelay, mShootCount);
    ...
    {
        auto& result(createEntity(mDrawPriority));
        result.createComponent<OBCParticleSystem>(mRenderTexture,
            game.getGameWindow(), mClearOnDraw, mOpacity, mBlendMode);
        return result;
    }
    ...
    Entity& OBFactory::createPit(const Vec2i& mPos)
    {
        ...
        gt<OBCPhys>(tpl).getBody().addGroups(
            OBGroup::GSolidGround, OBGroup::GPit);
        return gt<Entity>(tpl);
    }
    ...
    Entity& OBFactory::createPPlate(const Vec2i& mPos, int mId,
        PPlateType mType, IdAction mIdAction, bool mPlayerOnly)
    {
        auto tpl(createActorBase(mPos, {1000, 1000}, OBLayer::LFloor));
        const auto& intRect(
            mType == PPlateType::Single
                ? assets.pPlateSingle
                : (mType == PPlateType::Multi ? assets.pPlateMulti
                                              : assets.pPlateOnOff));
        emplaceSpriteByTile(gt<OBCDraw>(tpl), assets.txSmall, intRect);
        gt<Entity>(tpl).createComponent<OBCPPlate>(gt<OBCPhys>(tpl),
            gt<OBCDraw>(tpl), mId, mType, mIdAction, mPlayerOnly);
        return gt<Entity>(tpl);
    }
    Entity& OBFactory::createPlayer(const Vec2i& mPos)
    {
        ...
        auto& cWielder(
            gt<Entity>(tpl).createComponent<OBCWielder>(gt<OBCPhys>(tpl),
                gt<OBCDraw>(tpl), cDir8, assets.p1Stand, assets.p1Shoot));
    ...
    Entity& OBFactory::createSpawner(const Vec2i& mPos, SpawnerItem mType,
        int mId, float mDelayStart, float mDelaySpawn, int mSpawnCount)
    {
        auto tpl(createActorBase(mPos, {400, 400}, OBLayer::LShard));
        emplaceSpriteByTile(gt<OBCDraw>(tpl), assets.txSmall, assets.spawner);
        auto& cIdReceiver(gt<Entity>(tpl).createComponent<OBCIdReceiver>(mId));
        auto& cSpawner(gt<Entity>(tpl).createComponent<OBCSpawner>(
            gt<OBCPhys>(tpl), gt<OBCDraw>(tpl), cIdReceiver, mType, mDelayStart,
            mDelaySpawn, mSpawnCount));
    ...
    Entity& OBFactory::createBulletForceField(const Vec2i& mPos, int mId,
        Dir8 mDir, bool mBlockFriendly, bool mBlockEnemy)
    {
        auto tpl(createActorBase(mPos, {1000, 1000}, OBLayer::LWall, false));
        emplaceSpriteByTile(
    ...
    Entity& OBFactory::createEBall(
        const Vec2i& mPos, BallType mType, bool mSmall)
    {
        ...
        if(!mSmall && mType != BallType::Flying)
            gt<Entity>(tpl)
                ...
                .setActive(true);
        gt<Entity>(tpl).createComponent<OBCEBall>(
            gt<OBCEnemy>(tpl), mType, mSmall);
    ...
    Entity& OBFactory::createERunner(const Vec2i& mPos, RunnerType mType)
    {
        ...
        auto& cWielder(
            gt<Entity>(tpl).createComponent<OBCWielder>(gt<OBCPhys>(tpl),
                gt<OBCDraw>(tpl), cDir8, assets.e1Stand, assets.e1Shoot));
        auto& cWpnController(gt<Entity>(tpl).createComponent<OBCWpnController>(
    ...
    Entity& OBFactory::createECharger(const Vec2i& mPos, ChargerType mType)
    {
        ...
        auto& cWielder(
            gt<Entity>(tpl).createComponent<OBCWielder>(gt<OBCPhys>(tpl),
                gt<OBCDraw>(tpl), cDir8, assets.e2Stand, assets.e2Shoot));
        auto& cWpnController(gt<Entity>(tpl).createComponent<OBCWpnController>(
            gt<OBCPhys>(tpl), OBGroup::GFriendlyKillable));
    ...
    Entity& OBFactory::createEJuggernaut(
        const Vec2i& mPos, JuggernautType mType)
    {
        ...
        auto& cWielder(
            gt<Entity>(tpl).createComponent<OBCWielder>(gt<OBCPhys>(tpl),
                gt<OBCDraw>(tpl), cDir8, assets.e3Stand, assets.e3Shoot));
        auto& cWpnController(gt<Entity>(tpl).createComponent<OBCWpnController>(
            gt<OBCPhys>(tpl), OBGroup::GFriendlyKillable));
    ...
    Entity& OBFactory::createEGiant(const Vec2i& mPos)
    {
        ...
        gt<Entity>(tpl).createComponent<OBCFloorSmasher>(
            gt<OBCPhys>(tpl), true);
        ...
        OBCActorND* mShooter, const Vec2i& mPos, float mDeg)
    {
        ...
        gt<OBCProjectile>(tpl).onDestroy += [this, tpl, mShooter]
        {
            for(int i{0}; i < 360; i += 360 / 8)
            {
                ...
                Vec2i spawnPoint(
                    Vec2f(b.getOldPosition()) - b.getOldVelocity());
                gt<OBCProjectile>(tpl).createChild(createPJBoltPlasma(mShooter,
                    spawnPoint + Vec2i(ssvs::getVecFromDeg<float>(i) * 300.f),
                    i));
            }
        };
        gt<OBCDraw>(tpl).setBlendMode(sf::BlendAdd);
        ...
        OBCActorND* mShooter, const Vec2i& mPos, float mDeg, int mNum)
    {
        ...
        auto tpl(createProjectileBase(mShooter, mPos, {150, 150},
            560.f - (mNum * 70), mDeg, assets.pjShockwave));
        gt<Entity>(tpl).createComponent<OBCParticleEmitter>(
        ...
        if(mNum > 0)
        {
            gt<OBCProjectile>(tpl).onDestroy +=
                [this, tpl, mDeg, mNum, mShooter]
            {
                auto offset(ssvs::getVecFromDeg(mDeg + 180, 360));
                int num{2};
                for(int i{0}; i < num; ++i)
                    gt<OBCProjectile>(tpl).createChild(
                        createPJShockwave(mShooter,
    ...
    {
        auto tpl(createProjectileBase(mShooter, mPos, {150, 150},
            150.f * mSpeedMult, mDeg, assets.pjStar));
        gt<Entity>(tpl)
            .createComponent<OBCFloorSmasher>(gt<OBCPhys>(tpl))
            .setActive(true);
    ...
    Entity& OBFactory::createVMHealth(const Vec2i& mPos)
    {
        ...
        gt<OBCPhys>(tpl).getBody().addGroups(
            OBGroup::GSolidGround, OBGroup::GSolidAir);
        auto& cUsable(
            gt<Entity>(tpl).createComponent<OBCUsable>(gt<OBCPhys>(tpl)));
