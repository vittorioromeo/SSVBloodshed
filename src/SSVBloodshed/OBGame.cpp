...
namespace ob
{
	...
	void OBGame::newGame()
	{
		...
		psPerm = &factory.createParticleSystem(psPermTexture, false, 175, OBDrawPriority::OBDPParticleBlood).getComponent<OBCParticleSystem>().getParticleSystem();
		psTemp = &factory.createParticleSystem(psTempTexture, true, 255, OBDrawPriority::OBDPParticleTemp).getComponent<OBCParticleSystem>().getParticleSystem();
